...
def hill_climb(problem, max_iters=np.inf, restarts=0, init_state=None,
               curve=False, runtime=False, random_state=None):
    ...
    for _ in range(restarts + 1):
        ...
        if runtime:
            runtime_curve.append(process_time_ns() - start_time)

    best_fitness = problem.get_maximize()*best_fitness

    if curve and runtime:
        return best_state, best_fitness, np.asarray(fitness_curve), np.asarray(runtime_curve)
    elif curve and not runtime:
        return best_state, best_fitness, np.asarray(fitness_curve)
    elif not curve and runtime:
        return best_state, best_fitness, np.asarray(runtime_curve)

    return best_state, best_fitness


def random_hill_climb(problem, max_attempts=10, max_iters=np.inf, restarts=0,
                      init_state=None, curve=False, runtime=False, random_state=None):
...
def simulated_annealing(problem, schedule=GeomDecay(), max_attempts=10,
                        ...
                        runtime=False, random_state=None):
...
def genetic_alg(problem, pop_size=200, mutation_prob=0.1, crossover_prob=0.8, max_attempts=10,
                max_iters=np.inf, curve=False, runtime=False, random_state=None):
    ...
    while (attempts < max_attempts) and (iters < max_iters):
        ...
        for _ in range(pop_size):
            ...
            # Create offspring
            child = problem.reproduce(parent_1, parent_2, mutation_prob, crossover_prob)
