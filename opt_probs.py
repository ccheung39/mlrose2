...
class DiscreteOpt(OptProb):
    ...
    def reproduce(self, parent_1, parent_2, mutation_prob=0.1, crossover_prob=0.8):
        ...
        crossover_prob: float
            Probability of crossover at each state element during
            reproduction
        ...
        # Reproduce parents with crossover prob
        child = None
        randval = np.random.randint(len(parent_1))
        if randval > (1-crossover_prob)*len(parent_1):
            if self.length > 1:
                #half the time take parent 1 first, the other half take parent 2 first
                _n = np.random.randint(len(parent_1) - 1)
                child = np.array([0]*len(parent_1))
                whichfirst = np.random.uniform()
                if whichfirst <0.5:
                    child[0:_n+1] = parent_1[0:_n+1]
                    child[_n+1:] = parent_2[_n+1:]
                else:
                    child[0:_n+1] = parent_2[0:_n+1]
                    child[_n+1:] = parent_1[_n+1:]
        if child is None and np.random.randint(2) == 0:
